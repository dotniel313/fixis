...
| Security Regression | PENDING | RLS activo en ocho tablas y permisos RPC correctos; simulaciones SQL de customer, professional y admin pasan. 2026-09-26: `supabase/qa/FIXIS_RATINGS_THIRD_PARTY_READONLY.sql` devolvio tres `true` (cuenta ajena no ve identidad ni calificaciones). Accesos reales reportados en cinco telefonos, incluida misma cuenta en otro dispositivo; el usuario confirma funcionamiento tambien con otras cuentas reales. Falta evidencia explicita de que una cuenta no pueda ver datos ajenos en sesiones reales |
